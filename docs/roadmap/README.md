...
Turns the shared scheduling spec (`ros-launch-manifest-sched` + `check --sched`) from validate-now into apply: `--sched <file.toml>` + `--sched-apply {off,warn,strict}` sets `SCHED_FIFO`/`SCHED_RR` + priority + CPU affinity from the resolved `posix` tier — **per thread** (`/proc/<pid>/task/*` sweep; later threads inherit). Composables scheduled via `ComponentEvent.pid` + start-time identity (38.9). **Non-root** via the ROS-free `play_launch_rt_helper` holding `CAP_SYS_NICE` only (38.10) — the main binary must never carry a file capability (`AT_SECURE` would drop `LD_LIBRARY_PATH` and its ROS libs). Verified on-kernel, unprivileged: `fifo=11/11` on every node. `SCHED_DEADLINE` deferred *at the time* — **Phase 60 shipped it** (`sched_setattr(2)` replaced `sched_setscheduler(2)`, reservations derived under `reservations: required`), so read this entry as phase-38 history, not as the current apply layer. User guide: [docs/guide/rt-scheduling.md](../guide/rt-scheduling.md).
...
- **Phase 66** — ✅ cgroup-per-container (W1, W2, W4 shipped; W3 closed by
  ...
  - **Phase 70** — ✅ the consumer census: which fields are actually READ
    ...
    verified on a running system — `just jitter` in `rt_av_demo`, an oracle
    by construction: observed spread 6.03 ms against a true 6.0).
