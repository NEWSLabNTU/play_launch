...
## Phase 20.5: Type-Safe API Contract (ts-rs)

**Status**: Complete

Auto-generate TypeScript type declarations from Rust structs using
[ts-rs](https://github.com/Aleph-Alpha/ts-rs), then type-check the JavaScript
code against those declarations using `tsc --checkJs`. This prevents
format mismatches between the Rust backend's JSON serialization and the
JavaScript frontend's assumptions about data shapes.

### Motivation

Phase 20.0–20.4 introduced a bug where `getStatusString()` accessed
`status.value.status` on Process nodes, but serde serializes Process status
as `{"type": "Process", "value": "running"}` (value is a **string**, not
an object). Only Composable status uses the object format:
`{"type": "Composable", "value": {"status": "loaded"}}`. This format
mismatch caused Process and Container nodes to lose their status colors.

The root cause: no tooling enforced that the JS code matched the Rust
serde shapes. ts-rs bridges this gap — Rust types become the single
source of truth, and TypeScript catches shape mismatches at lint time.

### Design

```
Rust structs (web_types.rs, events.rs)
    │ #[derive(TS)] + #[ts(export)]
    ▼
TypeScript declarations (bindings/*.ts)     ← auto-generated by cargo test
    │ re-exported via types.d.ts barrel
    ▼
JSDoc annotations (@param, @type)           ← on critical JS functions
    │ checked by tsc --checkJs
    ▼
Type errors caught at lint time             ← e.g. "status.value.status"
                                               on a Process node is an error
```

### Types Exported

| Rust type              | Source file       | TypeScript output          |
|------------------------|-------------------|----------------------------|
| `NodeStatus`           | `web_types.rs`    | `"running" \| "stopped" \| ...` (string union) |
| `ComposableNodeStatus` | `web_types.rs`    | `{ "status": "loaded" } \| ...` (tagged union) |
| `UnifiedStatus`        | `web_types.rs`    | `{ "type": "Process", "value": NodeStatus } \| { "type": "Composable", "value": ComposableNodeStatus }` |
| `NodeSummary`          | `web_types.rs`    | Struct with all node fields |
| `NodeDetails`          | `web_types.rs`    | `NodeSummary & { cmdline? }` (flatten) |
| `NodeType`             | `web_types.rs`    | `"node" \| "container" \| "composable_node"` |
| `ComposableBlockReason`| `web_types.rs`    | `"container_stopped" \| ...` |
| `StateEvent`           | `events.rs`       | Internally-tagged union (10 variants) |
| `BlockReason`          | `state.rs`        | `"NotStarted" \| "Stopped" \| ...` |
| `HealthSummary`        | `web_query.rs`    | Struct with counter fields |

### Work Items

- [x] Add `ts-rs = "12"` to `play_launch` dev-dependencies
- [x] Add `#[cfg_attr(test, derive(TS))]` + `#[cfg_attr(test, ts(export))]`
  to all web-facing types (10 types across 4 files)
- [x] Handle `skip_serializing_if` fields with `#[ts(optional = nullable)]`
- [x] Override `u64` fields to `number` with `#[ts(type = "number")]`
- [x] Run `cargo test -- export_bindings` to generate `bindings/*.ts`
- [x] Create `types.d.ts` barrel in JS directory (re-exports from bindings)
- [x] Create `vendor.d.ts` ambient declarations for vendored Preact modules
- [x] Add `// @ts-nocheck` to vendor JS files (skip checking minified 3rd-party code)
- [x] Add JSDoc `@param`/`@type` annotations to critical functions:
  - `getStatusString(status: UnifiedStatus)` — the function that had the bug
  - `applyStateEvent(event: StateEvent)` — SSE event handler
  - `loadNodes(nodeArray: NodeSummary[])` — initial state load
  - `nodes` signal — typed as `Map<string, NodeSummary>`
  - `healthSummary` signal — typed as `HealthSummary`
- [x] Create `tsconfig.json` (`checkJs`, `moduleResolution: "bundler"`)
- [x] Add `typescript` npm dev-dependency
- [x] Add `npm run typecheck` script (`tsc --noEmit`)
- [x] Integrate into `npm run lint` pipeline

### Files Changed

| File                                  | Action                                                |
|---------------------------------------|-------------------------------------------------------|
| `src/play_launch/Cargo.toml`         | Add `ts-rs = "12"` dev-dependency                    |
| `src/web/web_types.rs`               | Add `#[derive(TS)]` to 7 types                      |
| `src/member_actor/events.rs`         | Add `#[derive(TS)]` to `StateEvent`                  |
| `src/member_actor/state.rs`          | Add `#[derive(TS)]` to `BlockReason`                 |
| `src/member_actor/web_query.rs`      | Add `#[derive(TS)]` to `HealthSummary`               |
| `src/play_launch/bindings/*.ts`      | New: 10 auto-generated TypeScript declarations       |
| `src/web/assets/js/types.d.ts`       | New: barrel re-exporting all generated types         |
| `src/web/assets/js/vendor.d.ts`      | New: ambient module declarations for Preact vendors  |
| `src/web/assets/js/store.js`         | Add JSDoc type annotations                           |
| `src/web/assets/js/vendor/*.js`      | Add `// @ts-nocheck`                                 |
| `tsconfig.json`                       | New: TypeScript config for `checkJs`                 |
| `package.json`                        | Add `typescript`, `typecheck` script                 |

### Passing Criteria

- [x] `cargo test -- export_bindings` generates 10 `.ts` files in `bindings/`
- [x] `npx tsc --noEmit` passes with zero errors
- [x] `UnifiedStatus` type correctly shows `Process.value` as `NodeStatus` (string)
  and `Composable.value` as `ComposableNodeStatus` (object) — the exact bug caught
...
| Type safety      | None (manual format matching)                 | Auto-generated TS types from Rust (ts-rs + tsc --checkJs) |
