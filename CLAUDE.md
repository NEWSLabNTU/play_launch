...
- **2026-01-17**: Logging cleanup - Removed all console.log debug statements from Web UI JavaScript and reviewed Rust logging levels to reduce noise. Changed info! to debug! for internal state transitions: "Updated auto_load for '{}' to {}" (user sees checkbox toggle in UI), "Transitioning {} composable nodes to Blocked state" (internal state detail), and "Transitioning to Pending for spawn" (internal state detail). These are now DEBUG-level logs that users can enable with `RUST_LOG=play_launch=debug` for troubleshooting. All user-facing actions (Start/Stop/Restart commands, Web UI button clicks) and major lifecycle events (container started/terminated, startup complete, respawn notifications) remain at INFO level. Result: Cleaner default output focused on essential user-facing information, with detailed technical logs available when needed for debugging (index.html:1232-1238, 1273, 1289, 1294, 1654, 1891, 1908, 1925; container_actor.rs:911, 1382, 1881; regular_node_actor.rs:416).
- **2026-01-17**: Fixed composable node log streaming - Added missing `node_type` and `container_name` fields to `/api/nodes/:name` endpoint JSON response, with dynamic container name lookup. **Root cause 1**: JavaScript correctly detected composable nodes and called setupLogTab with container info, but the API endpoint was missing these fields in the JSON, causing `node_type: undefined` and `container_name: undefined` in the browser. **Root cause 2**: After adding these fields, `container_name` contained the full ROS name with slashes (e.g., `/planning/.../container`), causing URL routing errors when trying to fetch logs from `/api/nodes//planning/.../logs/stderr` (slashes don't work in path parameters). **Solution**: (1) Added `node_type` field to get_node endpoint - converts NodeType enum to string ("Node", "Container", "ComposableNode") for JSON serialization (handlers.rs:493-497, 530). (2) Added `container_name` field to NodeSummary struct (web_types.rs:96), initialized to None (web_types.rs:146). (3) Implemented dynamic lookup in get_node handler - for composable nodes, searches all members to find the container with matching ROS name and returns its member name (the deduplicated name like "behavior_planning_container" without slashes) (handlers.rs:499-518, 526). Result: Composable nodes now correctly show parent container's stdout/stderr streams with blue reminder banner. Console logs show `isComposableNode: true` and valid `containerName` (member name, not ROS name). Log URLs use simple member names without slashes. Tested with Autoware simulation - all composable node logs working correctly (handlers.rs:486-538; web_types.rs:85-157).
- **2026-01-17**: Web UI improvements - multiple UX enhancements to the web interface: (1) View button styling - changed from gray to purple (#7c3aed) for better visibility. (2) Auto-switch panel - clicking any control button (Start/Stop/Restart/Load/Unload) on member Y now automatically switches the right panel to member Y while preserving the current tab. (3) Selected member highlighting - active member in right panel now highlighted with blue border in the list. (4) Composable node logs - composable nodes now show their parent container's logs with a blue reminder banner (setupLogTab function detects composable nodes and connects to container's log stream). (5) Tab preservation - switching members via View button now preserves the current tab (info/stdout/stderr) when panel is already open, defaults to stderr only when opening panel for first time. (6) State badge - right panel header now shows systemctl-style colored state badge (running=green, stopped=gray, failed=red, etc.). (7) Hierarchical sorting - added dropdown with 5 sorting options: Name A-Z, Name Z-A, Type, Status, Default (unsorted). Sorting preserves container/composable node tree structure - sorts containers/nodes at parent level, then sorts composable nodes within each container independently. Sort persists across refreshes. Changes: index.html (CSS at 676, 425-428, 322-338, 879-910; sorting controls at 952-960; hierarchical sort function at 1302-1378; tab preservation at 1208-1226; composable logs with debug logging at 1256-1289, 1452-1467; auto-switch at 1778-1828), handlers.rs (no changes needed - control flow via htmx events). Result: Cleaner UI with better visual feedback, easier member navigation, proper log streaming for composable nodes, and flexible list organization that respects hierarchy. Build successful: dist/play_launch-0.4.0-py3-none-any.whl.
- **2026-01-17**: Web UI redesign with three-tab panel and stream reconnection fixes - Merged Details and Logs buttons into single "View" button. Restructured right panel with three tabs: Member Info, stdout, stderr (stderr default). Fixed log streaming bugs: (1) Stream reconnection - Added PID tracking and automatic reconnection when node restarts (checks every 2s, reconnects both stdout/stderr if PID changes). (2) Jumping icon - Fixed stderr activity indicator to only show jumping animation when node is running (checks status-running/status-loaded classes). Each tab has independent log viewer with separate SSE connections, auto-scroll tracking, and controls. Changes: handlers.rs (merged buttons, showNodePanel function), index.html (tab structure at lines 873-877, tab CSS at 338-372, switchTab/connectToLog functions at 1057-1111, PID change detection at 1380-1407, jumping icon fix at 1546-1559). Result: Cleaner UI with unified view button, better log management with automatic reconnection on node restart, and accurate stderr activity indicator that stops jumping when node is stopped. Tested with Autoware simulation - all tab switching, log streaming, and reconnection working correctly.
