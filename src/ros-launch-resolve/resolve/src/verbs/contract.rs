//! `contract eject` — copy a package's provider contract (and matching
//! scheduling platform file, if any) into the overlay tree so a user can edit
//! them without ever touching `/opt` (design §3.3).

use std::path::{Path, PathBuf};

use eyre::{Result, eyre};

use crate::ros::{launch_dump::LaunchDump, manifest_loader, sched_loader};

/// Everything `contract eject` reads. Owned plain values — see [`crate::verbs`].
pub struct ContractEjectInputs {
    ...
    /// Which scheduling target's platform file to eject
    /// (`<stem>.system.<target>.yaml`).
    pub target: String,
    /// Overlay root to eject into. `None` = discover it the way
    /// `check --contracts` does.
    pub into: Option<PathBuf>,
    /// Overwrite existing overlay files.
    pub force: bool,
}

pub fn eject(inputs: ContractEjectInputs) -> Result<()> {
    let launch_path =
    ...
    let provider_platform =
        sched_loader::resolve_platform_provider_path(scope, &inputs.target).filter(|p| p.exists());
    ...
    let platform_dest = provider_platform.as_ref().and_then(|_| {
        sched_loader::resolve_platform_overlay_path(scope, &overlay_root, &inputs.target)
    });
